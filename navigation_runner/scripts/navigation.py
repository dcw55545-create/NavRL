...
class Navigation:
    def __init__(self, cfg):
        ...
        self.prev_vel_world = np.zeros(3)  # 用于 IMU 速度差分
        self.cmd_vel_world = np.zeros(3)       # ← 新增
        self.safe_cmd_vel_world = np.zeros(3)  # ← 新增
        ...
        self.collision_threshold = 0.25  # 略大于 robot_size=0.3
    ...
    def init_model(self):
        ...
        observation_spec = CompositeSpec({
            "agents": CompositeSpec({
                "observation": CompositeSpec({
                    ...
                    "imu": UnboundedContinuousTensorSpec((9,), device=self.cfg.device),  # ← 新增
                }),
            }).expand(1)
        ...
        policy.load_state_dict(torch.load(os.path.join(file_dir, checkpoint), map_location=self.cfg.device), strict=False)  # ← strict=False
        return policy
    ...
    def check_obstacle(self, lidar_scan, dyn_obs_states):
        ...
        #has_dynamic = False
        # print("has_static:", has_static)
        # print("has_dynamic:", has_dynamic)
        # print("dyn_obs_states:", dyn_obs_states.squeeze(0).squeeze(0))
        return has_static or has_dynamic
    # def check_obstacle(self, lidar_scan, dyn_obs_states):
    # # 静态：前方 ±40° 且 2m 以内
    #     eighth_size = lidar_scan.shape[2] // 8   # 36//8=4 beams
    #     close_val = self.cfg.sensor.lidar_range - 2.0  # 4-2=2.0
    #     has_static = (
    #         torch.any(lidar_scan[:, :, :eighth_size, 1:] > close_val) or
    #         torch.any(lidar_scan[:, :, -eighth_size:, 1:] > close_val)
    #     )
    #     # 动态：前方 (x>0.3) 且 2m 以内
    #     dyn = dyn_obs_states.squeeze(0).squeeze(0)
    #     valid_dyn = torch.any(dyn != 0., dim=-1)
    #     front_dyn = valid_dyn & (dyn[:, 0] > 0.3) & (dyn[:, 3] < 2.0)
    #     has_dynamic = torch.any(front_dyn)
    #     print("has_static:", has_static)
    #     print("has_dynamic:", has_dynamic)
    #     print("dyn_obs_states:", dyn_obs_states.squeeze(0).squeeze(0))
    #     return has_static or has_dynamic
    ...
    #     dyn = dyn_obs_states.squeeze(0).squeeze(0)
    #     valid_dyn = torch.any(dyn != 0., dim=-1)
    #     front_dyn = valid_dyn & (dyn[:, 0] > 0.3) & (dyn[:, 3] < 3.0)
    #     has_dynamic = torch.any(front_dyn)
    #     return has_static or has_dynamic
    ...
    def get_action(self, pos: torch.Tensor, vel: torch.Tensor, goal: torch.Tensor, imu: torch.Tensor):
        rpos = goal - pos
        ...
        # closest_dyn_obs_width = dynamic_obstacle_size[:, 0] / 0.25 - 1.
        # closest_dyn_obs_width[dynamic_obstacle_size[:, 2] == 0] = 0.
        # closest_dyn_obs_height = dynamic_obstacle_size[:, 2]
        # closest_dyn_obs_height = torch.where(
        #     closest_dyn_obs_height > 1.0,
        ...
        # print(f"has_obs={has_obstacle_in_range}, dyn_front={torch.any(torch.any(dyn_obs_states.squeeze(0).squeeze(0) != 0., dim=-1) & (dyn_obs_states.squeeze(0).squeeze(0)[:, 0] > 0.3))}")
        # print("has_obstacle_in_range:", has_obstacle_in_range)
        # print("cmd branch:", "policy" if has_obstacle_in_range else "direct_goal")
        # if (False):
    ...
    def control_callback(self, event):
        ...
        # ---- IMU 计算（9维：ang_vel_g + up_vec + lin_acc_g/10）----
        ang_vel_body_np = np.array([self.odom.twist.twist.angular.x,
                                    self.odom.twist.twist.angular.y,
                                    self.odom.twist.twist.angular.z])
        ang_vel_world_np = rot @ ang_vel_body_np          # 角速度 body->world
        up_vec_np = rot[:, 2]                             # 机体上方向（世界系）
        vel_world_np = rot @ vel_body
        acc_world_np = np.clip((vel_world_np - self.prev_vel_world) / 0.1, -10., 10.)  # dt=0.1s
        self.prev_vel_world = vel_world_np.copy()

        target_dir_imu = self.target_dir.clone()
        target_dir_imu[2] = 0.
        ang_vel_w_t = torch.tensor(ang_vel_world_np, dtype=torch.float, device=self.cfg.device).unsqueeze(0)
        acc_w_t     = torch.tensor(acc_world_np,     dtype=torch.float, device=self.cfg.device).unsqueeze(0)
        up_vec_t    = torch.tensor(up_vec_np,        dtype=torch.float, device=self.cfg.device)
        ang_vel_g   = vec_to_new_frame(ang_vel_w_t, target_dir_imu).squeeze()
        lin_acc_g   = vec_to_new_frame(acc_w_t,     target_dir_imu).squeeze()
        imu_tensor  = torch.cat([ang_vel_g, up_vec_t, lin_acc_g / 10.0]).unsqueeze(0)  # (1, 9)
        # ---- IMU 计算结束 ----

        # get RL action from model
        cmd_vel_world = self.get_action(pos, vel_world, goal, imu_tensor).squeeze(0).squeeze(0).detach().cpu().numpy()
        # print(f"cmd={cmd_vel_world[:2].round(2)}")
        self.cmd_vel_world = cmd_vel_world.copy()  # ← 新增
        #safe_cmd_vel_world = cmd_vel_world
        ...
        # print("cmd_vel_world:", cmd_vel_world)
        # print("safe_cmd_vel_world:", safe_cmd_vel_world)


        # 碰撞检测
        if self.mission_active and not self.collision_detected and len(self.raypoints) > 0:
            pos_np = np.array([self.odom.pose.pose.position.x,
                                self.odom.pose.pose.position.y,
                                self.odom.pose.pose.position.z])
            for rp in self.raypoints:
                if np.linalg.norm(np.array(rp) - pos_np) < self.collision_threshold:
                    self.collision_detected = True
                    self.mission_active = False
                    elapsed = (rospy.Time.now() - self.mission_start_time).to_sec()
                    print(f"[MISSION FAILED] 碰撞! t={elapsed:.1f}s, pos=({pos_np[0]:.2f},{pos_np[1]:.2f})")
                    # self._log_result("FAILURE", elapsed, pos_np)
                    break
        ...
        if (distance <= 3. and distance > 0.3):
            ...
            if self.mission_active and not self.mission_success:
                self.mission_success = True
                self.mission_active = False
                elapsed = (rospy.Time.now() - self.mission_start_time).to_sec()
                pos_np = pos.cpu().numpy()
                print(f"[MISSION SUCCESS] 到达目标! t={elapsed:.1f}s")
                # self._log_result("SUCCESS", elapsed, pos_np)
