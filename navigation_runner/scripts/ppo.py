...
from utils import ValueNorm, make_mlp, GAE, IndependentBeta, BetaActor, vec_to_world, ObstacleAttentionNetwork
...
class PPO(TensorDictModuleBase):
    def __init__(self, cfg, observation_spec, action_spec, device):
        ...
        # Dynamic obstacle with self-attention
        dynamic_obstacle_network = ObstacleAttentionNetwork(
            obs_dim=10, embed_dim=64, num_heads=4, output_dim=64
        ).to(self.device)

        # IMU feature extractor
        imu_network = make_mlp([64, 32]).to(self.device)
        ...
        self.feature_extractor = TensorDictSequential(
            ...
            TensorDictModule(imu_network, [("agents", "observation", "imu")], ["_imu_feature"]),
            CatTensors(["_cnn_feature", ("agents", "observation", "state"), "_dynamic_obstacle_feature", "_imu_feature"], "_feature", del_keys=False),
        ...
        def init_feature_(module):
            if isinstance(module, nn.Linear):
                nn.init.orthogonal_(module.weight, 1.0)
                nn.init.constant_(module.bias, 0.)

        self.feature_extractor.apply(init_feature_)
