...
class ObstacleAttentionNetwork(nn.Module):
    def __init__(self, obs_dim=10, embed_dim=64, num_heads=4, output_dim=64):
        super().__init__()
        self.embedding = nn.Linear(obs_dim, embed_dim)
        # 用手动点积注意力替代 nn.MultiheadAttention（兼容 torch.vmap）
    ...
    def forward(self, x):
        # x: (batch, 1, N_obs=5, 10)
        batch = x.shape[0]
        x = x.view(batch, -1, x.shape[-1])           # (batch, 5, 10)
        x = self.embedding(x)                          # (batch, 5, 64)
        q = self.q_proj(x)                             # (batch, 5, 64)
        k = self.k_proj(x)                             # (batch, 5, 64)
        v = self.v_proj(x)                             # (batch, 5, 64)
        attn = torch.bmm(q, k.transpose(-2, -1)) * self.scale  # (batch, 5, 5)
        attn = torch.softmax(attn, dim=-1)
        attn_out = torch.bmm(attn, v)                 # (batch, 5, 64)
        x = self.norm(x + attn_out)                   # 残差连接
        x = x.mean(dim=1)                             # (batch, 64)
        return self.output_proj(x)                    # (batch, 64)

class MultiModalFusionAttention(nn.Module):
    def __init__(self, embed_dim=256, num_heads=4):
        super().__init__()
        # 各模态投影到统一维度
        self.proj_lidar   = nn.LazyLinear(embed_dim)
        self.proj_state   = nn.LazyLinear(embed_dim)
        self.proj_imu     = nn.LazyLinear(embed_dim)
        self.proj_obs     = nn.LazyLinear(embed_dim)
        ...
        self.output = nn.Linear(embed_dim * 4, embed_dim)  # 4个模态展平

    def forward(self, lidar_feat, state_feat, imu_feat, obs_feat):
        # 各模态投影：每个 (batch, dim) → (batch, 1, 256)
        tokens = torch.stack([
            self.proj_lidar(lidar_feat),
            self.proj_state(state_feat),
            self.proj_imu(imu_feat),
            self.proj_obs(obs_feat),
        ], dim=1)                              # (batch, 4, 256)
        # 跨模态自注意力
        ...
        # 展平输出
        return self.output(tokens.flatten(1)) # (batch, 256)